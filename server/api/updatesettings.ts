...
export default async function handler(req: Request, res: Response<Data | Error>) {
	console.log("update settings received")
	...
	if (Object.keys(value).length===0) return res.status(400).json({error: "No settings to update"});

	const toUpdate: Partial<Settings> = {};
	if (value.avatar) {
		// todo: check if it's a valid url
		toUpdate.avatar = value.avatar;
	}
	...
	if (value.theme) {
		toUpdate.theme = value.theme;
	}
	users.updateOne({_id: user._id}, {$set: {settings: toUpdate}});
}
